 ...
 *
 * Los dominios de producción estaban escritos a mano en DOS sitios —aquí en
 * `src/config/index.js` y otra vez en `server.js`— y se aplicaban en todas las
 * casas. Consecuencia: la API de la casa Lite aceptaba peticiones con el Origin
 * del frontend de PRODUCCIÓN. Comprobado en vivo contra lite-api: devolvía 401,
 * no 403 — es decir, pasaba el filtro de CORS y solo la frenaba el token.
 *
...
describe('Aislamiento CORS entre casas', () => {
    test('en producción los orígenes salen SOLO de ALLOWED_ORIGINS', () => {
        const cfg = cargarConfig({
            NODE_ENV: 'production',
            ALLOWED_ORIGINS: 'https://app.mindloop.cloud,https://admin.mindloop.cloud',
        });
    ...
    test('en producción NO se cuela el frontend de otra casa', () => {
        ...
        expect(cfg.cors.allowedOrigins).not.toContain('https://lite.mindloop.cloud');
    ...
    test('el panel de admin sigue permitido si está en la variable', () => {
        // El panel admin SÍ habla con esta API. Con los dominios fuera del
        // código, que funcione depende de la variable: este caso lo deja
        // explícito para que nadie la recorte pensando que sobra.
        const cfg = cargarConfig({
            NODE_ENV: 'production',
            ALLOWED_ORIGINS: 'https://app.mindloop.cloud,https://admin.mindloop.cloud',
        });
        expect(cfg.cors.allowedOrigins).toContain('https://admin.mindloop.cloud');
    });
    ...
    test('ALLOWED_ORIGINS admite varios, con espacios sueltos', () => {
        // La variable se escribe a mano en Dokploy. Un espacio detrás de la coma
        // dejaba el origen fuera sin avisar de nada.
        const cfg = cargarConfig({
            NODE_ENV: 'production',
            ALLOWED_ORIGINS: 'https://app.mindloop.cloud, https://admin.mindloop.cloud ',
        });
