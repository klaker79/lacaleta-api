...
describe('Supplier price unit — pivot guarda €/unidad-base, no €/formato', () => {
    ...
    beforeAll(async () => {
        ...
        // `precio` es OBLIGATORIO aquí (resolverFormatoProveedor con requerirPrecio
        // = true); sin él el endpoint devuelve 400 y la relación no se crea.
        if (ingredientId && supplierId) {
            const linkRes = await request(API_URL)
                ...
                .send({
                    ...
                    precio: PRECIO_BASE_ESPERADO
                });
            if (![200, 201].includes(linkRes.status)) {
                console.warn('⚠️ Vinculación falló', linkRes.status, linkRes.body);
            }
        }
    });
    ...
    test('sin formato (cpf <= 1) el precio se propaga tal cual', async () => {
        ...
        await request(API_URL)
            ...
            .set('Origin', ORIGIN)
            .set('Authorization', `Bearer ${authToken}`)
            .send({ proveedor_id: supplierId, es_proveedor_principal: true, precio: 3.5 });
