...
module.exports = function (pool) {
    ...
    // NOTE: 404 catch-all handler stays in server.js (must be last middleware)
    ...
    router.get('/backup', authMiddleware, requireAdmin, async (req, res) => {
        try {
            ...
            const tables = [
                ...
                { name: 'proveedores', query: 'SELECT * FROM proveedores WHERE restaurante_id = $1 AND deleted_at IS NULL' },
                ...
                { name: 'inventory_snapshots', query: 'SELECT * FROM inventory_snapshots_v2 WHERE restaurante_id = $1 ORDER BY created_at DESC LIMIT 500' },
                { name: 'empleados', query: 'SELECT id, nombre, puesto, coste_hora, activo, restaurante_id FROM empleados WHERE restaurante_id = $1' },
